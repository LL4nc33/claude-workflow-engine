...
describe('StateManager', () => {
  ...
  describe('initialize', () => {
    ...
    it('should calculate checksums for installed files', () => {
  ...
  describe('getState', () => {
    ...
    it('should return null for corrupted state file', () => {
      writeTestFile(path.join(tempDir, '.installation-state.json'), 'not json');
      assert.strictEqual(stateManager.getState(), null);
    });
  });
  ...
  describe('getVersion', () => {
    ...
    it('should return version string when installed', () => {
      stateManager.initialize({
        mode: 'integrated',
        scope: 'local',
        profile: 'default',
  ...
  describe('createBackup', () => {
    it('should backup workflow directory', () => {
    ...
    it('should return null when no directories to backup', () => {
      stateManager.initialize({
        mode: 'integrated',
        scope: 'local',
        profile: 'default',
  ...
  describe('checkIntegrity', () => {
    ...
    it('should return empty arrays when not installed', () => {
      const result = stateManager.checkIntegrity();
  ...
  describe('cleanBackups', () => {
    it('should remove old backups keeping N most recent', () => {
      const backupDir = path.join(tempDir, '.workflow-backups');
      fs.mkdirSync(backupDir, { recursive: true });
    ...
    it('should not remove when backups <= keep count', () => {
      const backupDir = path.join(tempDir, '.workflow-backups');
      fs.mkdirSync(backupDir, { recursive: true });
      fs.writeFileSync(path.join(backupDir, 'a.backup-2024-01-01'), '');
  ...
  describe('rollback', () => {
    it('should fail when no backup available', () => {
      stateManager.initialize({
        mode: 'integrated',
        scope: 'local',
        profile: 'default',
    ...
    it('should fail when backup path does not exist', () => {
      stateManager.initialize({
        mode: 'integrated',
        scope: 'local',
        profile: 'default',
