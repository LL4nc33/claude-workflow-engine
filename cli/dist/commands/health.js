...
function checkHookScripts(targetPath, log, autoFix) {
    const checks = [];
    const fs = require('fs');
    const scriptsDir = path.join(targetPath, 'hooks', 'scripts');
    if (!(0, fs_utils_1.pathExists)(scriptsDir)) {
...
function checkSkillsCount(targetPath, log) {
    const checks = [];
    const fs = require('fs');
    const skillsDir = path.join(targetPath, '.claude', 'skills', 'workflow');
    if (!(0, fs_utils_1.pathExists)(skillsDir)) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.isDirectory()) {
                ...
                if ((0, fs_utils_1.pathExists)(skillFile)) {
                    skillCount++;
                }
            }
        }
