...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `workflow/standards/` (not in a subfolder).
...
Read `workflow/standards/index.yml` to get the list of available standards and their descriptions.
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `workflow/standards/api/`
- **Folder/file** — `api/response-format` → inject `workflow/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `workflow/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `workflow/standards/naming.md`
