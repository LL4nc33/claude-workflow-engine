...
import { GDPRValidator } from './gdpr-validator';
...
export class PreFlightChecker {
  ...
  run(): PreFlightReport {
    ...
    const detector = new ConflictDetector(this.options.path, templatePath, this.options.verbose);
    ...
    const gdpr = new GDPRValidator(this.options.path, this.options.verbose);
    ...
    for (const check of checks) {
      switch (check.status) {
        case 'pass': this.log.pass(check.message); break;
        case 'fail': this.log.fail(check.message); break;
        case 'warn': this.log.warn(check.message); break;
        case 'skip': this.log.skip(check.message); break;
      }
   ...
   */
  private checkPlatform(): CheckResult[] {
    const results: CheckResult[] = [];
   ...
   */
  private checkClaudeCode(): CheckResult {
    try {
      const { execSync } = require('child_process');
   ...
   */
  private checkTargetPath(): CheckResult[] {
    const results: CheckResult[] = [];
