...
Check if `workflow/product/` exists and contains files.

If it exists, read key files (like `mission.md`, `roadmap.md`, `tech-stack.md`) and use AskUserQuestion:

```
I found product context in workflow/product/. Should this feature align with any specific product goals or constraints?
...
Read `workflow/standards/index.yml` to identify relevant standards based on the feature being built.
...
**Note:** If `workflow/specs/` doesn't exist, create it when saving the spec folder.
...
Create `workflow/specs/{folder-name}/` with:
