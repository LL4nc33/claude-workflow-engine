...
- **Description:** `.claude-plugin/plugin.json` als Template in `cli/templates/base/` hinzufuegen
- **Agent:** debug
- **Dependencies:** None
- **Effort:** S
- **Acceptance Criteria:**
...
- **Description:** Alle Hook-Dateien als Templates in `cli/templates/base/hooks/` hinzufuegen
- **Agent:** debug
- **Dependencies:** None
- **Effort:** S
- **Acceptance Criteria:**
...
- **Description:** 6 Agent-Templates mit MCP-Tool-Referenzen und Usage-Sections aktualisieren (devops bleibt unveraendert)
- **Agent:** debug
- **Dependencies:** None
- **Effort:** M
- **Acceptance Criteria:**
...
- **Description:** 3 neue Plugin-Skills als Templates hinzufuegen
- **Agent:** debug
- **Dependencies:** None
- **Effort:** S
- **Acceptance Criteria:**
...
- **Effort:** S
- **Acceptance Criteria:**
  - [ ] Template enthaelt "Platform Architecture (6 Layers)" mit Diagramm
  - [ ] Template enthaelt "Available MCP Tools" Tabelle
  - [ ] Template enthaelt "Hook Behavior" Tabelle
  - [ ] Template enthaelt "Recommended MCP Servers" Abschnitt
...
- **Description:** config.yml auf v0.2.0 bumpen und `plugin:` sowie `mcp:` Sektionen hinzufuegen
- **Agent:** debug
- **Dependencies:** None
- **Effort:** S
- **Acceptance Criteria:**
...
- **Description:** `.claude-plugin`, `hooks`, `hooks/scripts` zu den erstellten Verzeichnissen hinzufuegen. Auch in dryRunInstall.
- **Agent:** debug
- **Dependencies:** 1.1, 1.2
- **Effort:** S
- **Acceptance Criteria:**
  - [ ] `dirs` Array enthaelt `.claude-plugin`, `hooks`, `hooks/scripts`
...
- **Description:** Nach dem Kopieren der Hook-Scripts `chmod +x` ausfuehren. pathMappings ggf. erweitern falls noetig (hooks/ und .claude-plugin/ haben keine Dot-Prefix-Probleme, sollten direkt kopiert werden).
- **Agent:** debug
- **Dependencies:** 2.1
- **Effort:** M
...
- **Acceptance Criteria:**
  ...
  - [ ] `fs.chmodSync(dest, 0o755)` fuer `.sh`-Dateien
...
- **Effort:** M
- **Acceptance Criteria:**
  - [ ] `coreFiles` enthaelt `.claude-plugin/plugin.json` (critical: true)
  - [ ] `coreFiles` enthaelt `hooks/hooks.json` (critical: true)
  - [ ] `requiredDirs` enthaelt `.claude-plugin`, `hooks`, `hooks/scripts`
  - [ ] Neue Check-Funktion: Hook-Scripts auf executable pruefen
  - [ ] Neue Check-Funktion: Skills-Anzahl pruefen (mind. 10 erwartet)
  - [ ] Auto-Fix: chmod +x fuer nicht-executable Hook-Scripts
- **Files to modify:**
...
- **Description:** Bestehende Tests auf v0.2.0 aktualisieren, neue Testfaelle fuer Hooks und Plugin hinzufuegen
- **Agent:** debug
- **Dependencies:** 4.1
- **Effort:** M
- **Acceptance Criteria:**
...
2. `npm test` -- alle Tests muessen bestehen
3. `node dist/index.js install --dry-run /tmp/test-project` -- zeigt neue Verzeichnisse und Dateien
4. `node dist/index.js install /tmp/test-project` -- installiert vollstaendig
