...
export async function resolveCommand(args: string[]): Promise<void> {
  ...
  const log = new Logger(true); // Always verbose for resolve
  ...
  const detector = new ConflictDetector(targetPath, templatePath, true);
...
async function resolveFileConflict(
  ...
  switch (conflict.type) {
    case 'modified': {
      ...
      const backup = createBackup(targetFile, backupDir);
      if (backup) {
      ...
      if (conflict.path.endsWith('.yml') || conflict.path.endsWith('.yaml')) {
        ...
        if (merged) {
          writeFileSafe(targetFile, merged);
 ...
 */
async function resolveCommandConflict(
  ...
  if (pathExists(conflict.existingSource)) {
    // Backup existing command
    const backup = createBackup(conflict.existingSource, backupDir);
    if (backup) {
 ...
 */
function showFileConflictOptions(conflict: FileConflict, log: Logger): void {
 ...
 */
function showCommandConflictOptions(conflict: CommandConflict, log: Logger): void {
  log.info('  Options:');
 ...
 */
function mergeYamlFiles(existingPath: string, templatePath: string): string | null {
  try {
    const existing = readFileSafe(existingPath);
    const template = readFileSafe(templatePath);

    if (!existing || !template) return null;
 ...
 */
function mergeJsonFiles(existingPath: string, templatePath: string): string | null {
  try {
    const existingContent = readFileSafe(existingPath);
    const templateContent = readFileSafe(templatePath);

    if (!existingContent || !templateContent) return null;
